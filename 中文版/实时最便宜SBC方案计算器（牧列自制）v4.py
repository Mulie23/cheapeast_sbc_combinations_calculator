...
def increase_rating():
    current_value = int(entry_target_rating.get())
    if current_value < 99:
        entry_target_rating.delete(0, tk.END)
        entry_target_rating.insert(0, str(current_value + 1))

def decrease_rating():
    current_value = int(entry_target_rating.get())
    if current_value > 0:  # prevent going into negative
        entry_target_rating.delete(0, tk.END)
        entry_target_rating.insert(0, str(current_value - 1))

# Define a function to save price data to a JSON file
def save_price_data(price_data):
    f = open('price_data.json', 'w')
...
def load_price_data():
    if os.path.exists('price_data.json'):
        f = open('price_data.json', 'r')
        price_info = json.load(f)
        f.close()
        return price_info
    else:
        return {"79": 400, "80": 400, "81": 400, "82": 450, "83": 900, "84": 2000, "85": 7600, "86": 13000, "87": 22250, "88": 30500, "89": 40750, "90": 58000, "91": 70000, "92": 111000}  # or return a default price
...
def update_player_prices():
    ...
    for rating in range(79, 93):
        ...
        player_prices[str(rating)] = price
    # print("haha")
    # print(player_prices)
    ...
    for rating in range(79, 93):
        ...
        price_entries[rating - 79].insert(0, player_prices.get(str(rating), ""))
...
# Tkinter GUI setup
root = tk.Tk()
root.title("实时最便宜SBC方案计算器(牧列自制)v4")
...
entry_target_rating = tk.Entry(root,width=6,justify="center")

plus_button = tk.Button(root, text="+1", width=5, borderwidth=2, command=increase_rating)
minus_button = tk.Button(root, text="-1", width=5, borderwidth=2, command=decrease_rating)
...
# Create a label with the text "牧列自制v4"
name_label = tk.Label(root, text="牧列自制v4")

# Place layout
label_players.place(x=10, y=16, anchor="w")
entry_players.place(x=200, y=16, anchor="w")

label_target_rating.place(x=405, y=16, anchor="w")
entry_target_rating.place(x=490, y=16, anchor="w")

plus_button.place(x=580, y=16, anchor="w")
minus_button.place(x=534, y=16, anchor="w")
